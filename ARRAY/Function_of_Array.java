...
class Function {
    ...
    void Insert_at_First(int size , int[] arr) {
        this.size = size;
        if (size <= 0) {
    ...
    void Insert_at_Last(int size , int[] arr) {
        if (size <= 0) {
            System.out.println("Please fill creat");
            Creat(size, arr);
        } else {
    ...
    void Insert_at_Anypotion(int size , int[] arr) {
        if (size <= 0) {
            System.out.println("Please fill creat");
            Creat(size, arr);
        } else {
            ...
            for (i = 0; i < size; i++) {
                temp [i/position] = arr[i/position];
            }
    ...
    public static void main(String[] args) {
        ...
        while(option)
        {
            ...
            switch ( n ) {
                ...
                case 3 -> F1.Insert_at_Last(size, arr);
                case 4 -> F1.Insert_at_Anypotion(size, arr);
                // case 5 -> F1.Delete_at_First(size, arr);
                // case 6 -> F1.Delete_at_Last(size, arr);
                // case 7 -> F1.Delete_at_Anypotion(size, arr);
